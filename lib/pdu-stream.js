...
class ParsingStream extends Transform {
    ...
    _transform(chunk, encoding, callback) {
        ...
        while (bufferPosition <= (buffer.length - 16)) {
            ...
            let pdu;
            try {
                pdu = this._pduParser.parse(pduBuffer);
            } catch (err) {
            ...
            // Moved push() out of try-catch since apparently it results in synchronously emitted 'data' event and
            // any exceptions thrown by listeners are intercepted by try-catch
            if (pdu) {
                this.push(pdu);
            }
