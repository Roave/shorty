...
function ServerConnection(socket, server, closeConnectionServerCallback, smppDefs) {
    const self = this;
    ...
    self.socketDataHandler = function (buffer) {
        ...
        for (let i = 0; i < pdus.length; i++) {
            ...
            if (self.bound === false) {
                if (pdu.command_id !== smpp.commands.bind_transceiver
                    && pdu.command_id !== smpp.commands.bind_receiver
                    && pdu.command_id !== smpp.commands.bind_transmitter
                ) {
            ...
            switch (pdu.command_id) {
                ...
                case smpp.commands.unbind_resp:
                    ...
                    break;
                default:
                    throw Error('unsupported command type');
            }
        }
    };

    self.error_response = function (pdu, status) {
        // @TODO is this supposed to send error response? Added return
        return {
    ...
    self.handleSubmitSm = function (pdu) {
        ...
        self.emit('submit_sm', self, pdu, (commandStatus, messageId) => {
    ...
    self.handleBind = function (pdu) {
        ...
        if (self.listeners('bind').length > 0) {
            self.emit('bind', self, pdu, status => self.bindAuthorization(pdu, status));
        } else {
...
exports.fromSocket = function (socket, server, connectionEndCallback, smppDefs) {
    const client = new ServerConnection(socket, server, connectionEndCallback, smppDefs);
