...
// @TODO states impl is a messy prototype. Refactoring is needed to make it consistent
...
class StateHandler {
     ...
     */
    sendCommand(pdu) {
        // @TODO resolve promise immediately if pdu does not have matching response
        ...
        return new Promise((resolve, reject) => {
            ...
            const resend = setTimeout(() => {
                ...
                this._context.pendingResponse.get(pdu.sequence_number).callback(
                    new errors.SmppSessionResponseTimeoutError(this._session.id, pdu)
                );
            }, PDU_RESP_WAIT_TIMEOUT);
            const callback = (err, pduResp) => {
                clearTimeout(resend);
                this._context.pendingResponse.delete(pdu.sequence_number);
                if (err) {
                    reject({ err, pdu });
                    return;
                } else if (pduResp.command_status !== 'ESME_ROK') {
                    reject(
                        new errors.SmppSessionResponseIsFailureError(this._session.id, pdu, pduResp)
                    );
                    return;
                }
                resolve(pduResp);
    ...
    handleIncomingResp(pdu) {
        ...
        // @TODO enforce pdu is either appropriate response or generic_nack
    ...
    sendInvalidStateRespFor(pdu) {
        const errPdu = smppUtils.errorResponse(this._smppDefs, pdu, 'ESME_RINVBNDSTS');
        this.sendResp(errPdu);
    }
...
/**
 * Initial state before session has started. Attempts to use session is a bug, hard fail
 */
class StateUninitialized extends StateHandler {
    sendCommand() {
        throw new errors.SmppSessionStateError(this._session.id, 'Session is not initialized');
    }
        ...
        switch (pdu.command) {
            ...
            case 'bind_transmitter':
                promise
                    .then(
                        (respPdu) => {
                            // @TODO assert response type is matching. Hard fail if not. See todo at response handling
                            let bindState = null;
                            switch (pdu.command) {
                                case 'bind_receiver':
                            ...
                            this._session.emit('bindSuccess', pdu, respPdu);
...
class StateServerOpen extends StateHandler {
    sendResp(pdu) {
        const promise = super.sendResp(pdu);
        switch (pdu.command) {
    ...
    handleBindResp(pdu) {
        ...
        if (pdu.command_status === 'ESME_ROK') {
            ...
            let bindState = null;
            switch (pdu.command) {
                // @TODO assert response is matching. Hard fail otherwise as it inidicates consumer induced bug
                case 'bind_receiver_resp':
                    bindState = 'stateBoundRx';
                    break;
                case 'bind_transmitter_resp':
        ...
        this._session.emit('bindFailure',
            new errors.SmppSessionError(
                this._session.id,
                `Bind rejected with status code ${pdu.command_status}`
            )
        );
        this._session.transitionStateHandler('stateClosed');
    }
...
class StateBound extends StateHandler {
    ...
    handleIncomingPdu(pdu) {
        // @TODO wait for pending commands to receive and send responses
        if (pdu.command === 'unbind') {
...
class StateClosed extends StateHandler {
    onEnter() {
        ...
        setImmediate(() => {
            this._context.pendingResponse.forEach((entry) => {
                entry.callback(new errors.SmppSessionStateError(
                    this._session.id,
                    'Session has closed before response was received',
                    this._session.state
                ));
            });
    ...
    sendCommand() {
        return Promise.reject(new errors.SmppSessionStateError(
            this._session.id,
