...
module.exports = exports = class PduParser {
    constructor(smppDefs) {
        exports.setSmppDefinitions(smppDefs);
    }

    parse(pduBuffer) {
        return exports.parse(pduBuffer);
    }
};

exports.setSmppDefinitions = function setSmppDefitions(defs) {
    smpp = defs;
};
...
exports.parse = function parse(buffer) {
    ...
    // Read the PDU header
    result.command_length = result.commandLength = buffer.readUInt32BE(offset);
    offset += 4;

    result.command_id = result.commandId = buffer.readUInt32BE(offset);
    result.command = smpp.command_ids[result.commandId];
    offset += 4;

    result.command_status = buffer.readUInt32BE(offset);
    result.command_status = smpp.command_status_codes[result.command_status] || 'ESME_RUNKNOWNERR';
    offset += 4;
    ...
    const format = smpp.command_formats[smpp.command_ids[result.command_id]];
        ...
        if (field.type === "int") {
            ...
            if (field.length === undefined) {
                ...
                if (buffer[offset] === 0x0) {
                    ...
                    const temp = new Buffer(end - start);
                ...
                if (buffer[offset] === 0x0) {
                    ...
                    const temp = new Buffer(field.length);
            ...
            const length = result[field.length_field];
            const temp = new Buffer(length);
