...
var serverClient = function(socket, server, closeConnectionServerCallback, callbacks) {
    ...
    self.deliver_sm = function(sender, recipient, message, user_ref) {
        self.sequence_number++;
        mySms = sms.create(sender, recipient, message, self.sequence_number, user_ref);

        myPdu = smpp.deliver_sm(mySms);
        self.sendPdu(myPdu);
    ...
    self.deliver_sm_resp = function(myPdu) {
        if (self.sms_delivered[ myPdu.sequence_number ] != undefined) {
            mySms = self.sms_delivered[ myPdu.sequence_number ];
    ...
    self.bind_transceiver_resp = function(myPdu) {
        ...
        if (bindAllowed) {
            ...
            if (self.config.strict == 1) {
                newPdu = smpp.bind_transceiver_resp(self.sequence_number, self.config.system_id, true);
            } else {
                self.bound = false;
                self.rejected = true;
                // slow down attackers
