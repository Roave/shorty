...
var serverClient = function(socket, server, closeConnectionServerCallback, callbacks) {
    ...
    self.socketDataHandler = function(buffer) {
        var parsedBuffer, myPdus, myPdu, i;
        ...
        // split PDUs
        parsedBuffer = pdu.fromBuffer(buffer, self.splitPacketBuffer);
        myPdus = parsedBuffer.pdus;
        self.splitPacketBuffer = parsedBuffer.splitPacketBuffer;

        // sanity check
        if (myPdus !== undefined) {
            for (i = 0; i < myPdus.length; i++) {
                myPdu = myPdus[i];
                ...
                if (self.pdusReceived[myPdu.command_id] === undefined) {
                    self.pdusReceived[myPdu.command_id] = 1;
                } else {
                    self.pdusReceived[myPdu.command_id]++;
                }
                ...
                if (self.bound === false) {
                    if (myPdu.command_id !== smpp.commands.bind_transceiver && myPdu.command_id !== smpp.commands.bind_receiver && myPdu.command_id !== smpp.commands.bind_transmitter) {
    ...
    self.submitResponse = function(mySms, success, message_id) {
        var newPdu;
        ...
        if (success === false) {
            newPdu = smpp.submit_sm_resp(mySms.sequence_number, message_id, false);
        } else {
            newPdu = smpp.submit_sm_resp(mySms.sequence_number, message_id, true);
        }
    ...
    self.bindAuthorizationComplete = function(myPdu, bindAllowed) {
         ...
         */
        if (bindAllowed) {
            newPdu = smpp.bind_resp(myPdu.command_id, myPdu.sequence_number, self.config.system_id);
            self.bound = true;
            ...
            if (self.config.strict === 1) {
                newPdu = smpp.bind_resp(myPdu.command_id, myPdu.sequence_number, self.config.system_id, true);
            } else {
    ...
    self.sendPdu = function(myPdu) {
        if (self.pdusSent[myPdu.command_id] === undefined) {
            self.pdusSent[myPdu.command_id] = 1;
        } else {
            self.pdusSent[myPdu.command_id]++;
        }
