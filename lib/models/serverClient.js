...
var serverClient = function(socket, server, closeConnectionServerCallback, callbacks) {
    ...
    self.deliverMessage = function(mySms) {
        self.deliver_sm(mySms);
    };
    ...
    self.handleSubmitSm = function(myPdu) {
        ...
        if (typeof self.callbacks['receiveOutgoing'] == 'function') {
            var message_id = self.callbacks['receiveOutgoing'](mySms);
        }

        if (message_id == false) {
            var newPdu = smpp.submit_sm_resp(myPdu, message_id, "failure");
        } else {
            var newPdu = smpp.submit_sm_resp(myPdu, message_id);
        }
    ...
    self.deliver_sm = function(mySms) {
        ...
        // similar to client.js, we keep track of SMS's we've tried to deliver
        self.sms_delivered[mySms.sequence_number] = mySms;
        mySms.failureTimeout = setTimeout(self.messageFailure, 20 * 1000, mySms.sequence_number);
    };

    self.messageFailure = function(sequence_number) {
        if (self.sms_delivered[ sequence_number ] != undefined) {
            delete self.sms_delivered[ sequence_number ];
            if ( DEBUG) { console.log("deliver_sm failed; seq: " + sequence_number); }
        }
    };

    self.handleDeliverSmResp = function(myPdu) {
        if (self.sms_delivered[ myPdu.sequence_number ] != undefined) {
            var mySms = self.sms_delivered[ myPdu.sequence_number ];

            clearTimeout(mySms.failureTimeout);
            ...
            if (typeof self.callbacks['deliverySuccess'] == 'function') {
                self.callbacks['deliverySuccess'](mySms);
            }

            // stop watching
            delete self.sms_delivered[ myPdu.sequence_number ];
        }
    };
    ...
    self.handleBind = function(myPdu) {
        ...
        if (typeof self.callbacks['bindRequest'] == 'function') {
            var bindAllowed = self.callbacks['bindRequest'](self.system_id, self.password, self.system_type, self.bind_type);
        } else {
