...
var sms = function(sender, recipient, message, sequence_number) {
    ...
    self.toPdu = function(command, convert) {
        var payload, message, iconv, encoding;

        if (convert !== undefined && convert === true) {
            iconv = new Iconv('UTF-8', 'UCS-2//TRANSLIT//IGNORE');
            message = iconv.convert(self.message);
            encoding = 0x08;
        } else {
            message = new Buffer(self.message);
            encoding = 0x03;
        }

        payload = pack.pack(
            'a1cca' + (self.sender.length + 1) + 'cca' + (self.recipient.length + 1) + 'ccca1a1cccccU' + (message.length),
            "",     //service_type
