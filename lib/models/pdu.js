...
var pdu = function(command_id, sequence_number, pdu_body, command_status) {
    ...
    self.getHeader = function() {
        return exports.createBuffer(exports.pack('NNNN', self.pdu_body.length + 16, self.command_id, 0, self.sequence_number));
    }
};
...
exports.fromBuffer = function(pduBuffer) {
    ...
    bufferPosition = 0;
    subBuffer = pduBuffer.slice(0, pduBuffer.length);

    while (subBuffer.length >= 16) {
        length = (subBuffer[0] << 24) +
                 (subBuffer[1] << 16) +
                 (subBuffer[2] << 8) +
                 (subBuffer[3]);

        myPdu = exports.individualPduFromBuffer(subBuffer.slice(0, length));
        pdus.push(myPdu);

        bufferPosition += length;
        subBuffer = pduBuffer.slice(bufferPosition, pduBuffer.length);
    }
