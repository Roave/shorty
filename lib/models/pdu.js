...
var pdu = function(command_id, sequence_number, pdu_body, command_status) {
    ...
    self.getHeader = function() {
        return exports.pack('NNNN', self.pdu_body.length + 16, self.command_id, self.command_status, self.sequence_number);
    }
};
...
exports.pack = function(format) {
    ...
    // first, we need to determine the total length (in bytes) our buffer needs to be
    ...
    var bufferPacked = new Buffer(bufferLength);
        ...
        switch (chr) {
            case 'A':
                ...
                for (j = 0; j <= num; j++) {
                    ...
                    if (j > arg.length) {
                        bufferPacked.write(' ', bufferOffset);
                        bufferOffset += 1;
                    } else {
                        bufferPacked.write(chrj, bufferOffset);
            ...
            case 'a':
                ...
                for (j = 0; j <= num; j++) {
                    var chrj = arg.charAt(j);
                    if (j > arg.length) {
                        bufferPacked.write("\0", bufferOffset);
                        bufferOffset += 1;
                    } else {
                        bufferPacked.write(chrj, bufferOffset);
                        bufferOffset += Buffer.byteLength(chrj);
            ...
            case 'c':
                ...
                num = parseInt(num);
                for (j = 1; j <= num; j++) {
                    bufferPacked.write(String.fromCharCode(arg), bufferOffset);
                    bufferOffset += Buffer.byteLength(String.fromCharCode(arg));
                    argi++;
                    var arg = arguments[argi];
                }
                break;
            case 'N':
                ...
                num = parseInt(num);
                for (j = 1; j <= num; j++) {
                    // put the octets into an array and initialize a new buffer
                    // with this array to avoid problems with node thinking that
                    // the argument is actually a UTF-8 character (avoid
                    // prepending of 0xc2)
                    var b = new Buffer([(arg >> 24) & 255, (arg >> 16) & 255, (arg >> 8) & 255, arg & 255]);
                    b.copy(bufferPacked, bufferOffset, 0);
                    bufferOffset += 4;
                    argi++;
                    var arg = arguments[argi];
                }
