...
exports.server = function(config) {
    ...
    self.deliverMessage = function(system_id, sender, recipient, message, user_ref) {
        for (var key in self.clients) {
            if (self.clients[key].system_id == system_id) {
                self.clients[key].deliver_sm(sender, recipient, message, user_ref);
            }
        }
    };
    ...
    self.connectionListener = function(incomingSocket) {
        ...
        myClient = client.fromSocket(incomingSocket, self, self.clientCloseHandler, self.clientCallbacks);
