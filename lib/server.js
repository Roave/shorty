...
exports.server = function Server(config, smppDefs) {
    const self = this;
    ...
    self.eventListeners = [];
    self.on('newListener', (event, listener) => self.eventListeners.push({ event, listener }));
     ...
     */
    self.deliverMessage = function (systemId, params, optionalParams) {
        for (const key in self.clients) {
            ...
            if (self.clients[key].system_id === systemId) {
                const id = self.clients[key].deliverMessage(params, optionalParams);
                if (id !== false) {
                    return id;
                }
            }
    ...
    self.connectionListener = function (incomingSocket) {
        ...
        const myClient = client.fromSocket(incomingSocket, self, self.clientCloseHandler, smpp);
    ...
    self.serverErrorHandler = function () {
        // noop
