...
exports.client = function(config) {
    ...
    self.tryNextIp = function() {
        if (self.config.local_ip !== undefined) {
            self.ip_index++;
            if (self.ip_index >= self.config.local_ip.length) {
                self.ip_index = 0;
            }
        }
    };
    ...
    self.connect = function() {
        ...
        if (self.config.local_ip !== undefined && self.config.local_ip.length > 0) {
            self.socket = net.createConnection(self.config.port, self.config.host, self.config.local_ip[self.ip_index]);
        } else {
        ...
        self.socket.on('data', function(buffer) {
            var parsedBuffer, myPdus, myPdu, i, message, d;
    ...
    self.deliver_sm_resp = function(myPdu) {
        ...
        newPdu = smpp.deliver_sm_resp(mySms.sequence_number);
        self.sendPdu(newPdu);
    ...
    self.socketErrorHandler = function(e) {
        switch(e.errno) {
            case 32:
                // Broken pipe
                //console.log('SOCKET ERROR (32): broken pipe');
                self.socket.end();
                break;
            case 104:
                //console.log('SOCKET ERROR (104): connection reset');
                self.socket.end();
                break;
            case 110:
                // connetion timed out
                ...
                break;
            case 111:
                //console.log('SOCKET ERROR (111): connection refused');
                self.socket.end();
                self.tryNextIp();
                break;
            case 112:
