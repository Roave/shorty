...
exports.client = function(config) {
     ...
     *  submit_sm that are awaiting a corresponding sm_submit_resp
     */
    self.sms_sent = {};
     ...
     *      - sent: fired when a submit_sm_resp is received for a message
     *      - send_failure: fired when a submit_sm_resp is not received within 20 seconds
     */
    self.callbacks = {};
    ...
    self.sendMessage = function(mySms) {
        ...
        // keep track of messages that should me marked as sent when a submit_sm_resp is received
    ...
    self.messageFailure = function(sequence_number) {
        if (self.sms_sent[ sequence_number ] != undefined) {
            if (typeof self.callbacks['send_failure'] == 'function') {
                self.callbacks['send_failure'](self.sms_sent[sequence_number]);
            }

            delete self.sms_sent[ sequence_number ];
            if ( DEBUG ) { console.log("submit_sm failed; seq: " + sequence_number); }
        }
    };
    ...
    self.submit_sm_resp = function(myPdu) {
        ...
        if (self.sms_sent[ myPdu.sequence_number ] != undefined) {
            ...
            if (myPdu.command_status == 0x0) {
                ...
                if (typeof self.callbacks['send_failure'] == 'function') {
                    self.callbacks['send_failure'](mySms);
                }
            }
